...
{
	return x2+y2+exp(v)-1-x*x-y*y-4.0/((1+x*x+y*y)*(1+x*x+y*y));
}

double dpde(double x, double y, double v, double x1, double y1, double x2, double y2,
	    double dv,double dx1,double dy1,double dx2,double dy2)
{
	return dx2+dy2+dv*exp(v);
}
}
